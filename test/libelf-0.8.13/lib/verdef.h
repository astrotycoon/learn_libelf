...
#ifndef lint
static const char verdef_h_rcsid[] =
    "@(#) $Id: verdef.h,v 1.13 2008/05/23 08:15:35 michael Exp $";
#endif				/* lint */
...
xlt_verdef(unsigned char *dst, const unsigned char *src, size_t n, unsigned enc)
{
	...
	while (off + sizeof(verdef_stype) <= n) {
		...
		if (vd.vd_version < 1 || vd.vd_version > VER_DEF_CURRENT) {
			seterr(ERROR_VERDEF_VERSION);
			return (size_t) - 1;
		}
		if (vd.vd_cnt < 1 || vd.vd_aux == 0) {
			seterr(ERROR_VERDEF_FORMAT);
			return (size_t) - 1;
		}
...
size_t
translator(verdef, L11) (unsigned char *dst, const unsigned char *src,
			 size_t n) {
	return xlt_verdef(dst, src, n, ELFDATA2LSB);
}

size_t
translator(verdef, M11) (unsigned char *dst, const unsigned char *src,
			 size_t n) {
