...
static const char rcsid[] =
    "@(#) $Id: gelftrans.c,v 1.10 2008/05/23 08:15:34 michael Exp $";
#endif				/* lint */
...
static char *get_addr_and_class(const Elf_Data * data, int ndx, Elf_Type type,
				unsigned *cls)
{
...
GElf_Rela *gelf_getrela(Elf_Data * src, int ndx, GElf_Rela * dst)
{
	...
	if (cls == ELFCLASS64) {
		*dst = *(Elf64_Rela *) tmp;
		...
		dst->r_info =
		    ELF64_R_INFO((Elf64_Xword) ELF32_R_SYM(src->r_info),
...
int gelf_update_rela(Elf_Data * dst, int ndx, GElf_Rela * src)
{
	...
	if (cls == ELFCLASS64) {
		*(Elf64_Rela *) tmp = *src;
		...
		dst->r_info =
		    ELF32_R_INFO((Elf32_Word) ELF64_R_SYM(src->r_info),
...
GElf_Move *gelf_getmove(Elf_Data * src, int ndx, GElf_Move * src)
{
	seterr(ERROR_UNIMPLEMENTED);
	return NULL;
}
